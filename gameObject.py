...
enemy_images = {'btr': {'btr_u': load_image(path_to_enemy + 'btr/up.png'),
                ...
                'heavy': {'heavy_u': load_image(path_to_enemy + 'heavy/up.png'),
                ...
                'rapidfire': {'rapidfire_u': load_image(path_to_enemy + 'rapidfire/up.png'),
                'rapidfire_d': load_image(path_to_enemy + 'rapidfire/down.png'),
                'rapidfire_l': load_image(path_to_enemy + 'rapidfire/left.png'),
                'rapidfire_r': load_image(path_to_enemy + 'rapidfire/right.png')},
                'standard': {'standard_u': load_image(path_to_enemy + 'standard/up.png'),
                'standard_d': load_image(path_to_enemy + 'standard/down.png'),
                'standard_l': load_image(path_to_enemy + 'standard/left.png'),
                'standard_r': load_image(path_to_enemy + 'standard/right.png')}}
...
class Brick(Tile):
    ...
    def __init__(self, posx, posy):
        super().__init__(Brick.brick_images['brick_0'], posx, posy, brick_group)
        self.state = 0 # 0 - 0 hit, 1 - 1 hit, 2 - 2 hit, 3 - 3 hit, 4 - destroy
    
    def update(self):
        if pygame.sprite.spritecollideany(self, bullet_enemy_group) or pygame.sprite.spritecollideany(self, bullet_player_group):
            self.state += 1
            if self.state == 4:
                self.kill()               
            else:
...
class Enemy(Tanks):
    ...
    bullet_speed = {'btr': 'standard', 'rapidfire': 'fast', 'standard': 'standard', 'heavy': 'standard'}
    spawn_place = [(0, 0), (6, 0), (12, 0)]
    ...
    def update(self):
        ...
        if enemy_rect.collidelist(list(bullet_player_group)) != -1 and self:
            self.hp -= 1
